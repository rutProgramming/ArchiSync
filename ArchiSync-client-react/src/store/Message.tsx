...
export const createMessage = createAsyncThunk(
  ...
  async ({ message }: { message: PartialMessage }, thunkAPI) => {
    try {
...
export const GetArchitectMessages = createAsyncThunk(
  'message/GetArchitectMessages',
  async (_, thunkAPI) => {
    try {
      const response = await axiosInstance.get("/api/Message/architect");
      return response.data;
    } catch (error: any) {
      return thunkAPI.rejectWithValue(error);
    }
...
export const UpdateMessageStatus = createAsyncThunk(
  'message/toggleMessageReadStatus',
  async (message: PartialMessage, thunkAPI) => {
    try {
      await axiosInstance.put(`/api/Message/${message.id}`, message);
      return message;
    } catch (error: any) {
      return thunkAPI.rejectWithValue(error);
    }
...
export const fetchUnreadMessagesCount = createAsyncThunk(
  'message/fetchUnreadMessagesCount',
  async (_, thunkAPI) => {
    try {
      const response = await axiosInstance.get("/api/Message/unread-count");
...
const messageSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(GetUserMessages.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string || 'Failed to fetch user messages';
      })
      ...
      .addCase(GetArchitectMessages.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string || 'Failed to fetch architect messages';
      })
      ...
      .addCase(fetchUnreadMessagesCount.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string || 'Failed to fetch unread message count';
      });
  },
});
